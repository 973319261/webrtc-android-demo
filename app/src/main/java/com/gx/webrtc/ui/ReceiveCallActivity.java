...
public class ReceiveCallActivity  extends AppCompatActivity {
    ...
    private EglBase mRootEglBase;//渲染API和本地窗口系统之间的接口（渲染视频）
    ...
    private VideoTrack localVideoTrack;//视频轨迹
    private AudioTrack localAudioTrack;//音频轨迹

    private CameraVideoCapturer mCameraVideoCapturer;//操作视频设备（调取摄像头）
    private int currVolume;//当前音量
    private CountDownTimer timer;//通话定时器
    ...
    private AudioManager mAudioManager;//媒体管理器
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        llReceive=findViewById(R.id.ll_call_receive);
        llDropped=findViewById(R.id.ll_call_dropped);
        ...
        localVideoSink.setTarget(mLocalSurfaceView);//设置对象
        //创建工厂
        mPeerConnectionFactory = createPeerConnectionFactory(this);

        // NOTE: this _must_ happen while PeerConnectionFactory is alive!
        Logging.enableLogToDebugOutput(Logging.Severity.LS_VERBOSE);
        //创建（视频设备）VideoCapturer
        ...
        localVideoTrack.addSink(localVideoSink); //将从摄像头采集的数据（视频轨迹）设置到该view里显示
        ...
        mLocalStream.addTrack(localAudioTrack);
        mLocalStream.addTrack(localVideoTrack);
        updateCallState(false,isVoice);
    ...
    private void setViewListener() {
        ...
        llReceive.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mSignalClient.responseCall(userId,remoteUid,RTCSignalClient.SIGNAL_TYPE_RECEIVE);//通知对方已经接受通话
        ...
        llDropped.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                close("取消通话");//关闭通话页面
                mSignalClient.responseCall(userId,remoteUid,RTCSignalClient.SIGNAL_TYPE_REJECT);//通知对方已挂断通话
            }
        });
        //取消
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (isCall) {//通话连接成功
                    ...
                    llDescribe.setVisibility(View.VISIBLE);//显示描述信息
                    llAwait.setVisibility(View.VISIBLE);//显示等待按钮
                    llCall.setVisibility(View.GONE);//显示通话中按钮
                    llCancel.setVisibility(View.GONE);//显示取消按钮
                    llVoice.setVisibility(View.GONE);//隐藏切换语音按钮
                    llCamera.setVisibility(View.GONE);//隐藏切换摄像头按钮
                    llMute.setVisibility(View.GONE);//隐藏禁麦按钮
                    llHands.setVisibility(View.GONE);//隐藏免提按钮
    ...
    public void doAnswerCall(String description) {
        ...
        //设置对端SDP
        mPeerConnection.setRemoteDescription(new CallActivity.SimpleSdpObserver(), new SessionDescription(SessionDescription.Type.OFFER, description));
        //媒体约束
        MediaConstraints sdpMediaConstraints = new MediaConstraints();
        MediaConstraints mediaConstraints = new MediaConstraints();
        ArrayList<MediaConstraints.KeyValuePair> keyValuePairs = new ArrayList<>();
        keyValuePairs.add(new MediaConstraints.KeyValuePair("OfferToReceiveAudio", "true"));// 告诉另一端，你是否想接收音频，默认true
        keyValuePairs.add(new MediaConstraints.KeyValuePair("OfferToReceiveVideo", "true"));// 告诉另一端，你是否想接收视频，默认true
        mediaConstraints.mandatory.addAll(keyValuePairs);
        Log.i(TAG, "Create answer ...");
        //创建Answer
        mPeerConnection.createAnswer(new CallActivity.SimpleSdpObserver() {
            @Override
            public void onCreateSuccess(SessionDescription sessionDescription) {
                Log.i(TAG, "Create answer success !");
                //设置本地SDP
                mPeerConnection.setLocalDescription(new CallActivity.SimpleSdpObserver(), sessionDescription);
                //发送Answer
                mSignalClient.sendAnswer(toJsonSessionDescription(sessionDescription), remoteUid);
            }
        }, sdpMediaConstraints);
    }
...
//        connection.addTrack(mAudioTrack);
   ...
   private RTCSignalClient.OnSignalEventListener mOnSignalEventListener=new RTCSignalClient.OnSignalEventListener() {
        ...
        @Override
        public void onVoice(JSONObject message) {//语音通话
            updateCallState(true,true);//切换到语音通话
           // mSignalClient.responseCall(userId,remoteUid,RTCSignalClient.SIGNAL_TYPE_VOICE);//通知对方已经切换语音通话
        }
        ...
        public void onOffer(JSONObject message) {//对方转发Offer
            Log.i(TAG,"Receive Remote Offer ...");
            if (mPeerConnection == null) {
                mPeerConnection = createPeerConnection();
            }
            try {
                ...
                doAnswerCall(description);//开始应答SDP
        ...
        public void onCandidate(JSONObject message) {//网络协商
            ...
            try {
                ...
                mPeerConnection.addIceCandidate(iceCandidate);//添加对方的网络信息
                updateCallState(true,isVoice);//连接成功
                if (startTime==0){
    ...
    protected void onResume() {
        super.onResume();
        if (isVoice){//语音通话
            tvCallState.setText("邀请你进行语音通话...");
            mCameraVideoCapturer.dispose();//关闭摄像头
        }else {//视频通话
            mLocalSurfaceView.setVisibility(View.GONE);
            mRemoteSurfaceView.setVisibility(View.GONE);
            tvCallState.setText("邀请你进行视频通话...");
        }
    }
    @Override
